...
// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object
// +kubebuilder:subresource:status
// +kubebuilder:path=karmadaonborad,scope=Namespaced,categories={karmada-io}
// +kubebuilder:printcolumn:JSONPath=`.spec.syncMode`,name="Mode",type=string
// +kubebuilder:printcolumn:JSONPath=`.metadata.creationTimestamp`,name="Age",type=date
type KarmadaOnborad struct {
	metav1.TypeMeta `json:",inline"`
	...
	// +optional
	Spec KarmadaOnboradSpec `json:"spec,omitempty"`

	// Most recently observed status of the KarmadaOnborad.
	// +optional
	Status KarmadaOnboradStatus `json:"status,omitempty"`
}
...
type KarmadaOnboradSpec struct {
	...
	// ClusterName describes the karmada cluster resource name.
	// +required
	ClusterName string `json:"clusterName"`

	// ClusterNamespace define namespace in the control plane where member secrets cluster are stored.
	// (default "karmada-cluster")
	// +optional
	ClusterNamespace string `json:"clusterNamespace"`

	// KarmadaControlPlane describes how to connect karmada control plane.
	// +optional
	KarmadaControlPlane *KarmadaControlPlane `json:"karmadaControlPlane"`

	// MemberCluster describes how to connect kubernetes cluster.
	// +optional
	MemberCluster *LocalSecretReference `json:"memberCluster"`

	// Agent holds settings to karmada-agent component of the karmada,
	// it only work on SyncMode Pull.
	// +optional
	Agent *AgentComponent `json:"agent"`

	// KarmadaSchedulerEstimator holds settings to karmada-scheduler-estimator component of the karmada.
	// +optional
...
// KarmadaSchedulerEstimatorComponent holds settings to karmada-scheduler-estimator component of the karmada.
type KarmadaSchedulerEstimatorComponent struct {
	// CommonSettings holds common settings to karmada scheduler estimator.
	CommonSettings `json:",inline"`
...
// AgentComponent holds settings to karmada-agent component of the karmada.
type AgentComponent struct {
	...
	//
	// For supported flags, please see
	// https://github.com/karmada-io/karmada/blob/master/cmd/agent/app/options/options.go
	// for details.
	// +optional
	ExtraArgs map[string]string `json:"extraArgs,omitempty"`
}
...
// KarmadaControlPlane describes how to connect karmada control plane.
type KarmadaControlPlane struct {
	// KarmadaName specifield name of karmadaDeployment, it represente
	// which karmada control plane to install.
	// +optional
	KarmadaName string `json:"karmadaName,omitempty"`
...
// Most recently observed status of the KarmadaOnborad.
type KarmadaOnboradStatus struct {
	// ObservedGeneration is the last observed generation.
	// +optional
	ObservedGeneration int64 `json:"observedGeneration,omitempty"`
...
// +kubebuilder:object:root=true
// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object
type KarmadaOnboradList struct {
	metav1.TypeMeta `json:",inline"`

	// +optional
	metav1.ListMeta `json:"metadata,omitempty"`

	Items []KarmadaOnborad `json:"items"`
}
